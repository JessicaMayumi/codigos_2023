...
def VerificarTelefone(telefone):
    while True:
        try:
            t = input(telefone)
            while len(t)!=11 or t.isdigit()==False:
                print("\033[31mNúmero de Telefone inválido, digite apenas 11 *números*!\033[m")
                t = input(telefone)
            strTelefone = ""
            count = 0
            for i in t:
...
def VerificarCEP(cep):
    while True:
        try:
            c = str(input(cep))
            while len(c)!=8 or c.isdigit()==False: 
                print("\033[31mCEP inválido, digite 8 *números*!\033[m")
                c = str(input(cep))
            strCEP = ""
            count = 0
            for i in c:
